...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        steps = 0
        while not nearest_place is hive:
            if steps in range(self.min_range, self.max_range + 1):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        copy_of_bees = self.place.bees.copy()
        self.armor -= amount 
        if self.armor <= 0:
            self.place.remove_insect(self) 
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.true_queen:
            previous = self.place.exit
            while previous != None:
                if previous.ant != None:
                    if (previous.ant.container and previous.ant.ant != None and 
                        not previous.ant.ant in self.damage_doubled): 
                        previous.ant.ant.damage *= 2
                        self.damage_doubled.append(previous.ant.ant)
                    elif not previous.ant in self.damage_doubled and previous.ant.damage != 0:
                        previous.ant.damage *= 2
                        self.damage_doubled.append(previous.ant)
    ...
    def reduce_armor(self, amount):
        ...
        if not self.armor:
            QueenAnt.num -= 1
        if self.true_queen and not self.armor:
            bees_win()
        # END Problem 13
