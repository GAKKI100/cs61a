...
def make_to_string(front, mid, back, empty_repr):
    ...
    def apply(link):
        if link == Link.empty:		
            return empty_repr
        return front + str(link.first) + mid + apply(link.rest)	+ back
    return apply
...
def long_paths(tree, n):
    ...
    for b in tree.branches:
        result += [Link(tree.label, r) for r in long_paths(b, n - 1)]
    return result 
