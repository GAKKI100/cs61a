...
def make_counter():
    ...
    def apply(x):
        nonlocal my_list
        my_list.append(x)
        return len([r for r in my_list if r == x])
    return apply
...
def make_withdraw(balance, password):
    ...
    attempt_list = []
    lock = False
    def apply(amount, attempt):
        nonlocal balance, lock
        if lock:
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, "x cannot be divided by zero"
...
def quadratic(x, a, b, c):
    ...
    x1 = -b / (2 * a)
    if lower <= x1 and upper >= x1:
        return interval(min(f(lower), f(upper), f(x1)), max(f(lower), f(upper), f(x1)))
    else:
...
def polynomial(x, c):
    ...
    "*** YOUR CODE HERE ***"
    lower, upper = lower_bound(x), upper_bound(x)
    f = lambda t: sum([c[k] * pow(t, k) for k in range(len(c))])
    df = lambda t: sum([c[k] * pow(t, k -1) * k for k in range(1, len(c))])
    ddf = lambda t: sum([c[k] * pow(t, k - 2) * k * (k - 1) for k in range(2, len(c))])
    num_steps = 20
